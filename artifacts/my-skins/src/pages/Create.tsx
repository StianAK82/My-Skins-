...
import {
  AvatarPreview,
  type GarmentConfig,
  type VisualEvidenceCapture,
} from "@/components/editor/AvatarPreview";
import { useDesignStore } from "@/lib/editor/design-state";
import {
  classicTextureAiSchema,
  parseClassicTextureAiPlan,
...
function mapAiModuleToLayer(module: {
  ...
  const isAccessory =
    module.type.toLowerCase().includes("accessory") ||
    module.type.toLowerCase().includes("hair");
  ...
  const halfWidth = 64;
  const halfHeight = 64;
  const layerType: "accessoryLayer" | "moduleLayer" = isAccessory
    ? "accessoryLayer"
    : "moduleLayer";
  return {
    name: module.label,
...
type GenerationEntitlementSummary = {
  freeFirst: "available" | "reserved" | "consumed" | "unavailable";
  availableGenerationCredits: { "2D": number; "3D": number };
  currentlyReserved: number;
  canGenerate: { "2D": boolean; "3D": boolean };
};
...
const DIRECT_ITEMS: Array<{
  key: keyof OutfitFiles;
...
type OutfitPlan = NonNullable<
  ReturnType<typeof normalizeAiResponse>["result"]["outfit"]
>;
...
type AccessorySlot =
  ...
  | "rightShoulder"
  | "aura";
const ACCESSORY_ASSET_MAP: Record<
  string,
  Array<{ slot: AccessorySlot; assetId: string }>
> = {
...
function diffOutfits(prev: OutfitPlan, next: OutfitPlan, t: Dict): string[] {
  ...
  if (prev.top !== next.top)
    changes.push(`${t.fieldTop}: ${name(prev.top)} → ${name(next.top)}`);
  if (prev.bottom !== next.bottom)
    changes.push(
      `${t.fieldBottom}: ${name(prev.bottom)} → ${name(next.bottom)}`,
    );
  if (prev.shoes !== next.shoes)
    changes.push(`${t.fieldShoes}: ${name(prev.shoes)} → ${name(next.shoes)}`);
  if (prev.hair.style !== next.hair.style)
    changes.push(
      `${t.fieldHair}: ${name(prev.hair.style)} → ${name(next.hair.style)}`,
    );
  else if (next.hair.style !== "none" && prev.hair.color !== next.hair.color)
    changes.push(t.itemNewColor(t.fieldHair));

  const prevAcc = new Map(
    prev.accessories.map((a) => [a.kind, `${a.color}:${a.size}`]),
  );
  const nextAcc = new Map(
    next.accessories.map((a) => [a.kind, `${a.color}:${a.size}`]),
  );
  for (const [kind] of prevAcc.entries())
    if (!nextAcc.has(kind)) changes.push(t.removed(name(kind)));
  for (const [kind, color] of nextAcc.entries()) {
    if (!prevAcc.has(kind)) changes.push(t.added(name(kind)));
    else if (prevAcc.get(kind) !== color)
      changes.push(t.itemNewColor(name(kind)));
  }

  const prevParts = new Map((prev.customParts ?? []).map((p) => [p.name, p]));
  const nextParts = new Map((next.customParts ?? []).map((p) => [p.name, p]));
  for (const [name] of prevParts.entries())
    if (!nextParts.has(name)) changes.push(t.removed(name));
  for (const [name, part] of nextParts.entries()) {
    if (!prevParts.has(name)) changes.push(t.added(name));
    else {
      const prevP = prevParts.get(name)!;
      if (
        prevP.color !== part.color ||
        prevP.size !== part.size ||
        prevP.attach !== part.attach
      ) {
...
function mapOutfitToSlots(
  outfit: OutfitPlan,
  t: Dict,
): {
  slots: Record<string, { assetId: string; color: string; scale: number }>;
  conflicts: string[];
} {
  const slots: Record<
    string,
    { assetId: string; color: string; scale: number }
  > = {};
  ...
  for (const acc of outfit.accessories) {
    ...
    if (taken) {
      conflicts.push(
        t.conflict(acc.kind, t.slotNames[taken.slot] ?? taken.slot),
      );
      continue;
    }
    for (const entry of mapped) {
      slots[entry.slot] = {
        ...
        color: acc.color,
        scale: accessoryPreviewScale(acc.size),
      };
    }
  }
...
function buildOutfitItemLists(
  outfit: OutfitPlan,
  conflicts: string[],
  t: Dict,
): { uploadable: string[]; previewOnly: string[] } {
  const name = (value: string) => t.itemNames[value] ?? value;
  const uploadable: string[] = [];
  const previewOnly: string[] = [];
  if (outfit.top !== "none")
    uploadable.push(`${t.fieldTop} (${name(outfit.top)})`);
  if (outfit.bottom !== "none")
    uploadable.push(`${t.fieldBottom} (${name(outfit.bottom)})`);
  if (outfit.shoes !== "none")
    previewOnly.push(`${t.fieldShoes} (${name(outfit.shoes)})`);
  if (outfit.hair.style !== "none")
    previewOnly.push(`${t.fieldHair} (${name(outfit.hair.style)})`);
  for (const acc of outfit.accessories) {
    if (
      !ACCESSORY_ASSET_MAP[acc.kind] ||
      conflicts.some((s) => s.includes(acc.kind))
    )
      continue;
...
export default function Create() {
  ...
  const [canonicalSpec, setCanonicalSpec] =
    useState<UniversalOutfitSpec | null>(null);
  ...
  const activePreviewScene = useMemo(
    () => (canonicalSpec ? toPreviewSceneSpec(canonicalSpec) : null),
    [canonicalSpec],
  );
  ...
  const [generationCredits, setGenerationCredits] =
    useState<GenerationEntitlementSummary | null>(null);
  ...
  useEffect(() => {
    if (
      !canonicalSpec ||
      geometryState !== "geometry_accepted" ||
      !previewTexture ||
    ...
    void (async () => {
      ...
      try {
        ...
        if (views.length !== 5)
          throw new Error("Incomplete five-view evidence");
        const response = await apiPost<{
          status: "READY" | "NEEDS_REPAIR" | "UNSUPPORTED" | "MANUAL_REVIEW";
          defects: string[];
        }>("/ai/visual-review", {
          generationId: canonicalSpec.generationId,
          attempt: 1,
          prompt: canonicalSpec.normalizedUserIntent,
          outfitSummary: canonicalSpec.items
            .map((item) => `${item.id}:${item.kind}`)
            .join(", "),
          itemIds: canonicalSpec.items.map((item) => item.id),
          classicExportValid: /^data:image\/png/.test(previewTexture),
          views,
        });
        if (cancelled) return;
        if (response.status !== 200)
          throw new Error("Visual review unavailable");
        if (response.data.status === "READY") setLifecycle("complete");
        else if (response.data.status === "UNSUPPORTED")
          setLifecycle("unsupported");
        else setLifecycle("external_verification_required");
        if (response.data.defects?.length)
          setAiError(response.data.defects.join(" · "));
        void apiGet<GenerationEntitlementSummary>(
          "/entitlements/generation-summary",
        )
          .then(setGenerationCredits)
          .catch(() => undefined);
      } catch {
        if (!cancelled) setLifecycle("external_verification_required");
        void apiGet<GenerationEntitlementSummary>(
          "/entitlements/generation-summary",
        )
          .then(setGenerationCredits)
  ...
  const [selectedHeadcover, setSelectedHeadcover] = useState<string | null>(
    null,
  );
  ...
  const headcoverOptions = (lastOutfit?.customParts ?? []).filter(
    (p) => p.shape === "headcover",
  );
  const shownHeadcover =
    selectedHeadcover === HEADCOVER_NONE
      ? HEADCOVER_NONE
      : (selectedHeadcover ?? headcoverOptions[0]?.name ?? null);
  const displayedCustomParts =
    headcoverOptions.length > 0
      ? lastOutfit?.customParts?.filter(
          (p) => p.shape !== "headcover" || p.name === shownHeadcover,
        )
  ...
  const refreshStatus = useCallback(async () => {
    ...
    try {
      setGenerationCredits(
        await apiGet<GenerationEntitlementSummary>(
          "/entitlements/generation-summary",
        ),
      );
    } catch {
  ...
  const redeemPromotion = async () => {
    ...
    const response = await apiPost<{ message?: string }>("/promotions/redeem", {
      code: promotionCode,
  ...
  const directUpload = useCallback(
    async (files: OutfitFiles): Promise<{ msg: string; allOk: boolean }> => {
      ...
      const uploadedPart =
        uploaded.length > 0 ? tt.sentDirectPrefix(uploaded.join(", ")) : "";
      return {
    ...
    if (params.get("robloxLogin") === "failed") {
      ...
      if (pendingDelivery) {
        ...
        void directUpload(pendingDelivery).then((result) => {
          if (result.allOk)
            window.localStorage.removeItem(PENDING_ROBLOX_DELIVERY_KEY);
        ...
        if (paid && pending) {
          ...
          if (consume.status === 200 && consume.data.ok) {
            ...
            if (me?.loggedIn) {
              ...
              setUploadStatus(
                `${tt.paidCreditsPrefix} ${tt.uploadDoneMsg} ${tt.paidNoDownloadHint}`,
              );
            }
          } else {
  ...
  useEffect(() => {
    ...
    const idHandle = window.setTimeout(() => {
      setPreviewTexture(
        renderDesignToCanvas(state, offscreen, {
  ...
  const generate = async (promptOverride?: string) => {
    ...
    try {
      ...
      const start = await apiPost<unknown>("/ai/generate", {
        prompt: usedPrompt,
      ...
      if (start.status !== 200) {
        const issue = start.data as { code?: string; message?: string };
        const error = new Error(
          issue.message ?? "generation failed",
        ) as Error & { status?: number; code?: string };
        error.status = start.status;
        error.code = issue.code;
        throw error;
      }
      const rawResponse = start.data as {
        result: unknown;
        outfitSpec?: unknown;
      ...
      const spec = universalOutfitSpecSchema.parse({
        ...(rawResponse.outfitSpec as object),
        creativeDirection: rawResponse.creativeDirection,
      });
      setLifecycle("routing");
      const previewScene = toPreviewSceneSpec(spec);
      if (previewScene.items.length === 0 && spec.items.length > 0)
        setLifecycle("unsupported");
      ...
      let topType: "hoodie" | "sweater" | "tshirt" | "jacket" | "dress" =
        "sweater";
      ...
      if (aiGarments) {
        ...
        const mentionsTop =
          /hettegenser|hette|hoodie|genser|sweater|skjort|sjort|shirt|jakke|jacket|topp|overdel|hood/i.test(
            combined,
          );
        const mentionsBottom =
          /shorts|bukse|olabukse|jeans|jogge|pants|underdel|dress|tracksuit/i.test(
            combined,
          );
        const mentionsShoes =
          /\bsko\b|joggesko|sneakers|boots|støvle|shoe/i.test(combined);
        const fullOutfit = !mentionsTop && !mentionsBottom && !mentionsShoes;
        wantsTop = mentionsTop || fullOutfit;
        wantsBottom =
          mentionsBottom ||
          (fullOutfit && aiUsesLegs) ||
          (fullOutfit && !aiText);
        wantsShoes = mentionsShoes;
        if (/hettegenser|hette|hoodie|hood/i.test(combined)) topType = "hoodie";
        else if (
          /tskjorte|t-skjorte|t-shirt|tshirt|skjort|sjort|shirt|topp/i.test(
            combined,
          )
        )
          topType = "tshirt";
      ...
      if (outfit) {
        ...
        setGarmentConfig({
          ...
          bottom:
            outfit.bottom === "none"
              ? null
              : (outfit.bottom as "pants" | "shorts" | "skirt"),
          shoes:
            outfit.shoes === "none"
              ? null
              : (outfit.shoes as "sneakers" | "boots"),
      ...
      const wantsCosmetics =
        /drage|dragon|ninja|superhelt|superhero|zombie|astronaut|romfar|hai|shark|lava|gamer|prinsesse|princess|enhjørning|unicorn|engel|angel|ving|wing|hjelm|helmet|caps|lue|hatt|hat\b|kjede|chain|krone|crown|horn/i.test(
          usedPrompt,
        );
      // Hairstyle should only change when the prompt actually asks for hair.
      const wantsHair =
        /hår|frisyre|hair|hestehale|ponytail|fletter|braid|krøll|curl|bob\b|panneluggen|lugg/i.test(
          usedPrompt,
        );
      if (!wantsHair) {
      ...
      if (outfit) {
        ...
        const { slots: accessorySlots, conflicts } = mapOutfitToSlots(
          outfit,
          t,
        );
        ...
        const { uploadable, previewOnly } = buildOutfitItemLists(
          outfit,
          conflicts,
          t,
        );
      ...
      const moduleLayers = response.result.modules.map((module) =>
        mapAiModuleToLayer(module),
      );
      ...
      applyAiPlan();
      if (response.result.colorPalette[0])
        setPaintSwatch(response.result.colorPalette[0]);
      ...
      outfitRef.current = {
        pantsBase: pantsColors.base,
        pantsAccent: pantsColors.accent,
      };
      if (wantsBottom) {
        for (const legZone of [
          "left_leg_front",
          "right_leg_front",
          "left_leg_back",
          "right_leg_back",
      ...
      if (spec.creativeDirection?.selected) {
        for (const layer of compileCreativeZoneArtwork(
          spec.creativeDirection.selected,
        ))
          addLayer(layer);
      }
      ...
      const wantsMotif =
        /logo|motiv|trykk|bilde|figur|mønster|print/i.test(pLow) ||
        wantsCosmetics;
      setAiPhase(t.phaseDrawing);
      const skipped = { status: 0, data: {} as { imageUrl?: string } };
      const [top, bottom, hero] = await Promise.all([
        wantsTop
          ? apiPost<{ imageUrl?: string }>("/ai/hero-image", {
              generationId,
              prompt: (outfit.topDescription ?? usedPrompt).slice(0, 600),
              kind: "garment-top",
            })
        ...
        wantsBottom
          ? apiPost<{ imageUrl?: string }>("/ai/hero-image", {
              generationId,
              prompt: (outfit.bottomDescription ?? usedPrompt).slice(0, 600),
              kind: "garment-bottom",
            })
          : Promise.resolve(skipped),
        wantsMotif
      ...
      const bottomUrl =
        bottom.status === 200 ? bottom.data.imageUrl : undefined;
      if (topUrl) {
      ...
      if (hero.status === 200 && hero.data.imageUrl) {
        ...
        if ((wantsTop && !topUrl) || (wantsBottom && !bottomUrl))
          setAiError(t.errMotif);
      }
      if (spec.items.every((item) => item.unsupported.state))
        setLifecycle("unsupported");
      else if (reviewedGenerationRef.current !== spec.generationId)
  ...
  const revise = async () => {
    ...
    try {
      ...
      if (res.status !== 200) {
        const err = new Error(`revise failed ${res.status}`) as Error & {
          status?: number;
          code?: string;
        };
      ...
      const candidateSpec = universalOutfitSpecSchema.parse(
        rawRevision.outfitSpec,
      );
      const revisedSpec = canonicalSpec
        ? reconcileCanonicalRevision(canonicalSpec, candidateSpec, text)
        : candidateSpec;
      setLifecycle("routing");
      ...
      const hairChanged =
        lastOutfit.hair.style !== outfit.hair.style ||
        lastOutfit.hair.color !== outfit.hair.color;
      if (hairChanged) {
        const hairAssetId =
          outfit.hair.style !== "none"
            ? HAIR_ASSET_MAP[outfit.hair.style]
            : undefined;
        setAvatarSlot(
          "hair",
          hairAssetId
      ...
      const topChanged =
        ...
        lastOutfit.topDescription !== outfit.topDescription;
      const bottomChanged =
        lastOutfit.bottom !== outfit.bottom ||
        lastOutfit.bottomDescription !== outfit.bottomDescription;
      const combinedPrompt = `${lastPromptRef.current}. Endring: ${text}`.slice(
        0,
        600,
      );
      ...
      if (topChanged || bottomChanged) {
        ...
        const [top, bottom] = await Promise.all([
          topChanged && outfit.top !== "none"
            ? apiPost<{ imageUrl?: string }>("/ai/hero-image", {
                generationId: candidateSpec.generationId,
                prompt: (outfit.topDescription ?? combinedPrompt).slice(0, 600),
                kind: "garment-top",
              })
            : Promise.resolve(skipped),
          bottomChanged && outfit.bottom !== "none"
            ? apiPost<{ imageUrl?: string }>("/ai/hero-image", {
                generationId: candidateSpec.generationId,
                prompt: (outfit.bottomDescription ?? combinedPrompt).slice(
                  0,
                  600,
                ),
                kind: "garment-bottom",
        ...
        if (topChanged) {
          for (const layer of layersNow().filter(
            (l) => l.name === "AI-overdel",
          ))
        ...
        if (bottomChanged) {
          for (const layer of layersNow().filter(
            (l) => l.name === "AI-bukse" || l.name === "Bukse",
          ))
            deleteLayer(layer.id);
          const bottomUrl =
            bottom.status === 200 ? bottom.data.imageUrl : undefined;
          if (outfit.bottom !== "none") {
            ...
            outfitRef.current = {
              pantsBase: pantsColors.base,
              pantsAccent: pantsColors.accent,
              heroUrl: outfitRef.current?.heroUrl,
              fabricUrl: bottomUrl,
            };
          } else {
            outfitRef.current = outfitRef.current
              ? { ...outfitRef.current, fabricUrl: undefined }
              : null;
          }
        }
      }
      ...
      setSelectedHeadcover((current) =>
        ...
        (current &&
          (outfit.customParts ?? []).some(
            (p) => p.shape === "headcover" && p.name === current,
          ))
          ? current
          : null,
      );
      ...
      setUndoStack((stack) => [
        ...stack.slice(-(UNDO_STACK_LIMIT - 1)),
        snapshot,
      ]);
      setLifecycle("external_verification_required");
    } catch (error) {
  ...
  const undoLast = () => {
    ...
    reviewedGenerationRef.current = null;
    setLifecycle(
      snapshot.canonicalSpec ? "external_verification_required" : "idle",
    );
    setSelectedHeadcover(null);
  ...
  const renderExportFiles = async (): Promise<OutfitFiles | null> => {
    ...
    const eligible = new Set(
      canonicalSpec.items
        ...
        .map((item) => item.exportCapability),
    );
    if (!eligible.has("classic_shirt") && !eligible.has("classic_pants"))
      return null;
    await preloadOverlayImages(state);
    const shirt = renderDesignToCanvas(state, canvas, {
      onOverlayImageReady: handleOverlayImageReady,
      target: "export",
    });
    ...
    const pants = await renderPantsTexture({
      base: pantsBase,
      accent: pantsAccent,
      motifUrl: outfit?.heroUrl,
      fabricUrl: outfit?.fabricUrl,
    });
    const tshirt = outfit?.heroUrl
      ? await renderTShirtTexture(outfit.heroUrl)
      : undefined;
    return {
  ...
  const uploadToRoblox = async () => {
    ...
    try {
      ...
      const consume = await apiPost<{ ok?: boolean; needsPayment?: boolean }>(
        "/payments/consume-free",
      );
      if (consume.status === 200 && consume.data.ok) {
        setReadyFiles(files);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-3xl px-4 py-8 flex flex-col items-center gap-8">
        <header className="relative w-full text-center space-y-2">
              ...
              <button
                ...
                className={`rounded-lg border px-2 py-1 text-lg transition ${
                  lang === l.code
                    ...
                    : "border-slate-700 bg-slate-900/70 opacity-60 hover:opacity-100"
                }`}
              >
                {l.flag}
              </button>
          ...
          <p data-testid="create-tagline" className="text-lg text-slate-300">
            {t.tagline}
          </p>
        </header>
            ...
            {t.ideas.map((idea, index) => {
              ...
              return (
                ...
                >
                  <IdeaIcon
                    aria-hidden="true"
                    className="h-10 w-10 text-emerald-300 sm:h-12 sm:w-12"
                    strokeWidth={1.7}
                  />
        ...
        <section className="w-full">
            ...
            <div className="mt-4 rounded-xl border border-slate-700 bg-slate-900/60 p-4 text-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {headcoverOptions.map((part) => (
                  ...
                  >
                    <span
                      className="h-3 w-3 rounded-full border border-slate-500/50"
          ...
          {canonicalSpec && (
            <div
              data-testid="canonical-result"
              data-generation-id={canonicalSpec.generationId}
              data-item-ids={canonicalSpec.items
                .map((item) => item.id)
                .join(",")}
            ...
            >
              {lifecycle === "complete"
                ...
                : lifecycle === "unsupported"
                  ? "Denne ideen kan vi ikke vise ennå."
                  : lifecycle === "external_verification_required"
                    ? "Vi må sjekke denne litt ekstra."
                    : aiPhase}
            </div>
          )}
          {canonicalSpec && (
            <div
              data-testid="geometry-acceptance"
              data-state={geometryState}
              className="sr-only"
            >
              {geometryState === "geometry_accepted"
                ? "Your outfit is ready."
                : geometryState === "geometry_limited" ||
                    geometryState === "geometry_rejected"
                  ? "This part could not be shown correctly."
                  : "I’m checking the outfit."}
            </div>
          )}
          {outfitItems && (
            <div
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {outfitItems.changed?.map((item, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs"
                      >
                        {item}
                      </span>
                    ))}
              ...
              {outfitItems.uploadable.length > 0 && (
                <div>
                  ...
                  <p className="text-xs text-slate-400 mb-2">
                    {t.goesIntoRobloxHint}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {outfitItems.uploadable.map((item, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-xs"
                      >
                        {item}
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {outfitItems.previewOnly.map((item, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 rounded-full bg-sky-500/20 text-sky-300 border border-sky-500/30 text-xs"
                      >
                        {item}
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {outfitItems.unsupported.map((item, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 rounded-full bg-slate-700/40 text-slate-400 border border-slate-600/40 text-xs"
                      >
                        {item}
                      </span>
                    ))}
        ...
        <section className="w-full space-y-4">
          {aiLoading ? (
            <div className="flex flex-col items-center gap-2 rounded-2xl border-2 border-emerald-500/50 bg-emerald-500/10 p-5 text-center">
              ...
              <p className="text-lg font-semibold">
                🎨 {aiPhase || t.loadingDefault}
              </p>
              <p className="text-sm text-slate-300">{t.loadingWait}</p>
            </div>
          ) : null}
          {aiError ? (
            <p className="text-center text-sm text-red-400">{aiError}</p>
          ) : null}
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            {robloxMe?.configured ? (
              robloxMe.loggedIn ? (
                <p className="text-sm text-slate-300">
                  {t.loggedInAsPrefix}{" "}
                  <span className="font-semibold">{robloxMe.name}</span>{" "}
                  {t.loggedInAsSuffix}{" "}
                  <button
                    type="button"
                    className="underline text-slate-400 hover:text-slate-200"
                    onClick={() => {
                      void apiPost("/auth/roblox/logout").then(() =>
                        setRobloxMe({ loggedIn: false, configured: true }),
                      );
                    }}
                  >
                    {t.logout}
            ...
            {skinStatus ? (
              <p className="text-sm text-slate-400">
                {skinStatus.paidCredits > 0
                  ? t.creditsLeft(skinStatus.paidCredits)
                  : t.priceInfo}
              </p>
            ) : null}
            {uploadStatus ? (
              <p className="text-sm text-emerald-400 text-center max-w-lg">
                {uploadStatus}
              </p>
            ) : null}
            {readyFiles ? (
            ...
            {!hasDesign && !aiLoading ? (
              <p className="text-sm text-slate-500">{t.tapHint}</p>
            ) : null}
          </div>
            ...
            <div className="w-full rounded-xl border-2 border-sky-500/40 bg-sky-500/5 p-4">
              ...
              >
                ...
                <Button
                  type="submit"
                  size="lg"
                  className="h-12 px-6 bg-sky-500 text-sky-950 hover:bg-sky-400"
                  disabled={aiLoading || !reviseText.trim()}
                >
          ...
          <details className="w-full rounded-xl border border-slate-800 bg-slate-900/50 p-4">
            ...
            >
              <Input
                ...
                disabled={aiLoading}
              />
              <div className="flex gap-1" aria-label="Preview type">
                <Button
                  type="button"
                  variant={generationMode === "2D" ? "default" : "outline"}
                ...
                <Button
                  type="button"
                  variant={generationMode === "3D" ? "default" : "outline"}
              ...
              <Button
                ...
                disabled={
                  aiLoading ||
                  !prompt.trim() ||
                  generationCredits?.canGenerate[generationMode] === false
                }
              >
                <Sparkles className="mr-2 h-5 w-5" />
                {t.createButton}
            ...
            {generationCredits ? (
              <p
                ...
                data-testid="generation-credit-message"
              >
                {generationCredits.freeFirst === "available"
                  ? "Your first skin is free."
                  : generationCredits.availableGenerationCredits[
                        generationMode
                      ] > 0
                    ? `You have ${generationCredits.availableGenerationCredits[generationMode]} skins left.`
                    : generationCredits.currentlyReserved > 0
                      ? "We are fixing your skin. You will not be charged again."
      ...
      {show3dGuide ? (
        ...
        >
          ...
          >
            <h2 className="mb-3 text-xl font-bold text-violet-300">
              {t.guideTitle}
            </h2>
            ...
            <p className="mb-5 rounded-lg border border-slate-700 bg-slate-800/60 p-3 text-xs text-slate-400">
              {t.guideNote}
            </p>
