...
import { AiGenerationError, normalizeAiError, timeoutMs, withAiTimeout } from "./ai-errors.ts";
...
const defaults: Dependencies = {
  ...
  generate: async (...args) => {
    const { generateClassicTexture } = await import("./classic-texture.service.ts");
...
export async function generateCompleteOutfit(description: string, dependencies: Partial<Dependencies> = {}) {
  const deps = { ...defaults, ...dependencies };
  const generationId = randomUUID();
  const startedAt = Date.now();
  let stage = "blueprint";
  const emit = (event: string, extra: Record<string, unknown> = {}) => deps.log(event, {
    generationId,
  ...
  try {
    return await withAiTimeout(async () => {
      emit("complete_outfit.blueprint_started", { openAIRequestType: "none" });
      const outfitBlueprint = deps.plan(description);
      emit("complete_outfit.blueprint_completed", { openAIRequestType: "none" });
      ...
      emit("complete_outfit.shirt_started", { openAIRequestType: "images.edit", attempt: 1 });
      const shirt = await deps.generate("shirt", shirtPrompt, shirtSpecification, diagnostics);
      emit("complete_outfit.shirt_completed", { openAIRequestType: "images.edit", attempt: shirt.attempts });

      stage = "pants_generation";
      emit("complete_outfit.pants_started", { openAIRequestType: "images.edit", attempt: 1 });
      const pants = await deps.generate("pants", pantsPrompt, pantsSpecification, diagnostics);
      emit("complete_outfit.pants_completed", { openAIRequestType: "images.edit", attempt: pants.attempts });
      ...
      return result;
    }, timeoutMs("COMPLETE_OUTFIT_TIMEOUT_MS", 260_000), stage);
  } catch (error) {
    const normalized = error instanceof AiGenerationError ? error : normalizeAiError(error, stage);
    const failure = normalized.code === "AI_TIMEOUT" && normalized.stage === "blueprint" && stage !== "blueprint"
      ? new AiGenerationError(normalized.message, normalized.code, stage, normalized.retryable, normalized.status)
      : normalized;
