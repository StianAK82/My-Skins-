...
test("semantic classification produces a panel-aware garment blueprint", () => {
  const blueprint = enhanceGarmentPrompt(
    "shirt",
    "comfortable premium black sweater with a red dragon",
    {
  ...
  assert.deepEqual(blueprint.colourProfile, {
    primary: ["black"],
    accent: ["red"],
  });
  assert.ok(
    blueprint.front.some((detail) => detail.includes("kangaroo pocket")),
  );
  assert.ok(blueprint.back.some((detail) => detail.includes("hood")));
  assert.ok(blueprint.sleeves.some((detail) => detail.includes("sleeves")));
  assert.equal(blueprint.designMemory.garmentKey, "oversized-hoodie");
  assert.deepEqual(blueprint.decorativeDetails, [
    "dragon",
    "red dragon on front",
  ]);
  ...
  assert.match(prompt, /DESIGN MEMORY \(oversized-hoodie\)/);
});
