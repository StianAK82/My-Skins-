...
- The "wall through the avatar" bug had TWO causes: (1) drei <Environment>+<Lightformer> panels leaked into the visible scene as giant colored walls (replaced with plain ambient/directional lights, incl. pink/blue rim), and (2) the Stage floor cylinderGeometry had rotation-x=-PI/2 which turned the flat disc into a vertical wall — cylinderGeometry is already flat, never rotate it for a floor. Reproduce side-view bugs by orbiting with mouse-drag in the headless script.
- Render-polish lessons (July 2026 studio upgrade): textured garment materials must use color #ffffff — a tinted base color multiplies the map and turns outfits murky. RoomEnvironment PMREM (local, no CDN) is bright: set scene.environmentIntensity ~0.35 and dispose the render target on unmount or GPU memory leaks. Skin must stay matte (no clearcoat, roughness ~0.6) and total light must stay under the Bloom threshold or faces glow. LOW_END_DEVICE flag drops shadow samples + bloom on tablets.
