...
The representative run produces 82 screenshots: six fixed desktop views for each of 13 outfit cases, plus a narrow-mobile front view for the first four cases. `test-results/visual-evidence.json` records every prompt, camera view, viewport and artifact path. CI checks the manifest count and rejects missing or empty files before uploading the screenshot and report artifacts with 30-day retention. This prevents a green visual job from silently publishing an incomplete evidence set. The uploaded screenshot artifact also contains `visual-evidence.html`, a responsive gallery for reviewing all camera cases without opening 82 files individually; the Actions job summary records the rendered commit and verified count.

The first captured browser set exposed missing emoji glyphs as empty square boxes in the idea grid and primary call-to-action copy. Idea artwork is now rendered with bundled SVG icons, and the visual smoke test checks representative presets for those SVGs so the result no longer depends on fonts installed on the Actions runner.
